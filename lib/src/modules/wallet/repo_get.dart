...
getWalletBalanceRepo(
  ...
  if (responseCheck) {
    Get.find<WalletLogic>().getWalletBalanceModel = GetWalletBalanceModel.fromJson(response);
...
getWalletTransactionRepo(
    BuildContext context, bool responseCheck, Map<String, dynamic> response) {
  if (responseCheck) {
    Get.find<WalletLogic>().getAllTransactionModel = GetAllTransactionModel.fromJson(response);
    ...
    if (Get.find<WalletLogic>().getAllTransactionModel.status == true) {
      ...
      if (Get.find<WalletLogic>()
          .getAllTransactionModel.data!.transactions!
          .isNotEmpty) {
        for (var element in Get.find<WalletLogic>()
            .getAllTransactionModel.data!.transactions!) {
          Get.find<WalletLogic>()
              .updateGetAllTransactionList(element);
        }
