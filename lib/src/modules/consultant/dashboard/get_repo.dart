...
getMentorApprovalRepo(
  ...
  if (responseCheck) {
    ...
    if (Get.find<DashboardLogic>()
        ...
        true) {
      if(Get.find<DashboardLogic>()
          .mentorApprovalCheckModel.data!.mentor!.isProfileCompleted == 0){
        Get.offAllNamed(PageRoutes.createConsultantProfile);

      }else{
        if(Get.find<DashboardLogic>()
            .mentorApprovalCheckModel.data!.mentor!.status == 1){
          Get.find<DashboardLogic>().updateApprovalCheckerApiStopLoader(true);
        }
        Get.find<DashboardLogic>()
            .updateApprovalCheckerApiLoader(false);
      }

      log('getMentorApprovalRepo ------>> ${Get.find<DashboardLogic>().mentorApprovalCheckModel.success}');
    } else {
      Get.find<DashboardLogic>()
          .updateApprovalCheckerApiLoader(false);
    }
